	...
	BEGIN 
		FOR i IN 0..4 LOOP 
			partition_week := format(
				'create table if not exists measures_week_%s partition of measures for values from (''%s'') to (''%s'') partition by range (ts)',
			...
			FOR j IN 0..6 LOOP
				partition_days := format(
					'create table if not exists measures_week_%s_day_%s partition of measures_week_%s for values from (''%s'') to (''%s'')',
					to_char(now() + make_interval(weeks => i), 'YYYY_MM_DD'),
